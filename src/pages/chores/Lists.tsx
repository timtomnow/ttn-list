...
export function ChoreLists() {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (newIdx === -1) return;
    await reorderChoreList(String(active.id), newIdx);
  };
              ...
              {filtered.map((list) => {
                ...
                const subtitle =
                  list.entries.length === 0
                    ? 'empty'
                    : `${itemEntries} chore${itemEntries === 1 ? '' : 's'}, ${routineEntries} routine${routineEntries === 1 ? '' : 's'}`;
                const runButton = ip ? (
                  <Link
                    to={`${list.id}/run`}
                  ...
                  >
                    <Hourglass size={13} /> Continue ({ip.resolvedItems.filter((r) => r.checked).length}/{ip.resolvedItems.length})
                  </Link>
                ) : (
                  <Link
                    to={`${list.id}/run`}
