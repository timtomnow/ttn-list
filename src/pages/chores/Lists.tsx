...
import { deleteChoreList, reorderChoreList, useChoreLists, useInProgressChoreSessions } from '@/db/repo';
import type { ChoreList, ChoreSession } from '@/types';

export function ChoreLists() {
  const lists = useChoreLists();
  const inProgress = useInProgressChoreSessions();
  const toast = useToast();
  const [query, setQuery] = useState('');

  const inProgressByList = useMemo(() => {
    const m = new Map<string, ChoreSession>();
  ...
  const onMove = async (list: ChoreList, dir: -1 | 1) => {
    const idx = sorted.findIndex((s) => s.id === list.id);
    const next = idx + dir;
    if (next < 0 || next >= sorted.length) return;
    await reorderChoreList(list.id, next);
  };

  const onDelete = async (list: ChoreList) => {
    if (!confirm(`Delete "${list.name}"? Past sessions for this list are kept.`)) return;
    await deleteChoreList(list.id);
    toast.show('Deleted');
  };
    ...
    <div>
      ...
      {sorted.length === 0 ? (
        <EmptyState
          ...
          title="No lists yet"
          description="A list bundles chores and routines for one run. Create one and tap Chore it to step through it."
        ...
        <ul className="space-y-2">
          {filtered.map((list) => {
            ...
            const routineEntries = list.entries.length - itemEntries;
            return (
              <li key={list.id} className="flex items-center gap-3 rounded-2xl border border-ink-200 bg-white p-3 dark:border-ink-800 dark:bg-ink-900">
                <Link to={list.id} className="min-w-0 flex-1">
                  ...
                  <div className="truncate text-xs text-ink-500 dark:text-ink-400">
                    {list.entries.length === 0 ? 'empty' : `${itemEntries} chore${itemEntries === 1 ? '' : 's'}, ${routineEntries} routine${routineEntries === 1 ? '' : 's'}`}
                  </div>
                </Link>
                {(() => {
                  ...
                  return (
                    <Link to={`${list.id}/run`} className="btn-secondary h-9 px-3 text-xs" aria-label={`Run ${list.name}`}>
                      <Play size={14} /> Run
                    </Link>
                  );
                })()}
