...
export function ChoreHistoryDetail() {
  const { id } = useParams();
  const session = useChoreSession(id);
  const items = useChoreItems();
  const list = useChoreList(session?.listId);
  ...
  if (!session) {
    return <div><Link to="/chores/history" className="text-sm text-ink-500"><ChevronLeft size={14} /> History</Link><EmptyState title="Session not found" description="It may have been deleted." action={<Link to="/chores/history" className="btn-primary">Back to history</Link>} /></div>;
  }

  if (session.completedAt === undefined) {
    return (
      <div>
        <Link to="/chores/lists" className="text-sm text-ink-500"><ChevronLeft size={14} /> Lists</Link>
        <EmptyState title="Still in progress" description="This run hasn't been wrapped up yet. Continue from the Lists page."
          action={<Link to={`/chores/lists/${session.listId}/run`} className="btn-primary">Continue</Link>} />
      </div>
    );
  }
  ...
  const onDelete = async () => {
    ...
    await deleteChoreSession(session.id);
    toast.show('Session deleted');
    navigate('/chores/history');
  };

  const onRerun = () => { if (!listExists) return; navigate(`/chores/lists/${session.listId}/run`); };
  ...
  return (
    <div>
      <Link to="/chores/history" className="mb-3 inline-flex items-center gap-1 text-sm text-ink-500 hover:text-ink-900 dark:text-ink-400 dark:hover:text-ink-50"><ChevronLeft size={16} /> History</Link>
      ...
      <div className="space-y-5">
        ...
        <section className="card p-5">
          <div className="mb-3 text-sm font-medium">Chores</div>
